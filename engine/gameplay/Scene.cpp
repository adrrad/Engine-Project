...
GameObject* Scene::FindOrInstantiateGameObject(Engine::GUID id)
{
...
GameObject* Scene::InstantiateGameObject()
{
    ...
    GameObject* go = new (m_allocator->Allocate<GameObject>()) GameObject(id, this);
...
{
   Gameplay::ComponentManager::GetInstance()->Update(deltaTime);
}
