...
_forceinline PxTransform Convert(Gameplay::Transform t) 
{ 
    PxTransform trans;
...
_forceinline Gameplay::Transform Convert(PxTransform t) 
{ 
    Gameplay::Transform trans;
...
RigidBody* PhysicsManager::CreateRigidBody(Gameplay::Transform &transform, std::vector<ColliderInfo> colliders, float mass, void* owner)
{   
    PxShape* shape = GetCollisionShape(colliders[0]);
    PxRigidDynamic* actor = mPhysics->createRigidDynamic(Convert(transform));
    if(colliders[0].Type == Physics::ColliderType::TERRAIN) actor->setRigidBodyFlag(PxRigidBodyFlag::eKINEMATIC, true);
