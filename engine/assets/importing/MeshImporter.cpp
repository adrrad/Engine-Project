...
MeshData* MeshImporter::ImportMesh(Platform::IO::File* meshFile)
{
    ...
    for(u64 meshIndex = 0; meshIndex < meshes.size(); meshIndex++)
    {
        auto indices = meshes[meshIndex].mesh.indices;
        auto posIndices = std::vector<u64>(indices.size());
        auto normIndices = std::vector<u64>(indices.size());
        auto uvIndices = std::vector<u64>(indices.size());
