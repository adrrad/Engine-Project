...
void ShaderAsset::Load()
{
    ...
    if(!vert) AssertFail("Shader asset loading error: Vertex shader asset '" + m_vertexShaderID.ToString() + "' does not exist!");
    if(!frag) AssertFail("Shader asset loading error: Fragment shader asset '" + m_fragmentShaderID.ToString() + "' does not exist!");
