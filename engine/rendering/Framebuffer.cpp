...
void Framebuffer::CreateFBO(std::vector<std::string> colorbuffers, std::vector<std::string> depthbuffers, std::vector<Format> formats)
{
    ...
    Array<BufferHandle> cbHandles(cbCount);
    Array<BufferHandle> dbHandles(dbCount);
    Array<uint32_t> attachments(cbCount);
    bool isRebuilding = m_colorNames.size() > 0;
    glGenFramebuffers(1, &m_fbo);
    glBindFramebuffer(GL_FRAMEBUFFER, m_fbo);
    // Colorbuffers
    glGenTextures(cbCount, cbHandles.Data());
    glGenTextures(dbCount, dbHandles.Data());
    ...
    //Set as draw buffers 
    glDrawBuffers(cbCount, attachments.Data());
...
{
    if(Default == nullptr) Default = EngineSubsystem<Renderer>::GetSubsystemAllocator()->New<Framebuffer>();
    return Default;
}
